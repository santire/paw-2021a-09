        ...
        <c:when test="${loggedUser}">
            <c:if test="${rated}">
                ...
                <form action="${path}" method="post">
                    ...
                    <input id="ratedRating" name="rating" class="rating-loading" value="${restaurant.getRating()}" data-min="0" data-max="5" data-step="0.5">
                </form>
            </c:if>
            <c:if test="${!rated}">
                ...
                <form action="${path}" method="post">
                    ...
                    <input id="unratedRating" name="rating" class="rating rating-loading" data-min="0" data-max="5" data-step="0.5">
                </form>
            </c:if>
        </c:when>
        ...
        <c:when test="${!loggedUser}">
            ...
            <script>
                $(document).ready(function(){
                    $('displayOnlyRating').rating({displayOnly: true, step: 0.5});
                });
            </script>
        </c:when>
    </c:choose>
